# d:\programas\DDS\dds-webtools\boletim_x_ponto\scripts\migrate_parquet_to_firestore.py
import os
import sys
...
def upload_collection_in_batches(subcollection_name: str, records: list, doc_id_field: str | None = None):
    print(f"Uploading {len(records)} records to subcollection 'webtools/boletim_x_ponto/{subcollection_name}'...")
    parent_doc = db.collection("webtools").document("boletim_x_ponto")
...
def main():
    ...
    db.collection("webtools").document("boletim_x_ponto").set(meta_doc)
    print("Metadata written successfully to webtools/boletim_x_ponto:")
