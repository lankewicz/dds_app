...
def inject_revision():
    rev = (os.getenv("K_REVISION") or os.getenv("APP_REVISION") or "").strip()
    m = re.search(r"-(\d{5})-", rev)
    short = m.group(1) if m else (rev or "local")
    return {"app_revision": rev or "local", "app_revision_short": short}
