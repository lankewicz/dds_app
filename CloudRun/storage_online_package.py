...
def delete_online_package_and_update_lista_json(
    *,
    ...
    for attempt in (1, 2):
        files, gen = _download_lista_json(bucket, lista_path)
        before = len(files)
        files = [p for p in files if not p.startswith(prefix)]
        removed_from_lista = before - len(files)
        try:
            _upload_lista_json_atomic(bucket, lista_path, files, if_generation_match=gen)
            break
        except Exception:
            if attempt == 2:
                raise
...
def create_or_update_online_package(
    *,
    ...
    for attempt in (1, 2):
        files, gen = _download_lista_json(bucket, lista_path)
        files = _merge_paths(files, new_paths)
        try:
            _upload_lista_json_atomic(bucket, lista_path, files, if_generation_match=gen)
            break
        except Exception:
...
def list_all_sessions_from_storage(
    *,
    ...
    """
    # Note: list_online_packages_from_lista_json must be defined elsewhere or imported
